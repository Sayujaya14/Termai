...
- skill value must be an exact filename from the catalog (e.g. eda_report.md).
...
def _normalize_route(data: dict | None, *, has_upload: bool) -> TaskRoute:
    ...
    if skill_file in (None, "null", ""):
        ...
        if load_skill_instructions(skill_file) is None:
            skill_file = None
    else:
        skill_file = None
...
def classify_task(
    ...
    *,
    ...
    """
    catalog = format_skill_catalog() or "(none)"
    ...
    try:
        ...
        route = _normalize_route(data, has_upload=has_upload)
    except Exception:
